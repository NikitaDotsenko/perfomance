...
class XHProfRuns_Default implements iXHProfRuns {
  ...
  protected function db()
  {
	...
	require_once XHPROF_LIB_ROOT.'/utils/Db/'.xhprof['dbadapter'].'.php';

    $class = self::getDbClass();
    $this->db = new $class(xhprof);
    $this->db->connect();
  }
...
*/
  ...
  */
  public function getRuns($stats)
  {
      ...
      {
        $query = "SELECT {$stats['select']} FROM `xhprof_details` ";
      }else
      {
        $query = "SELECT * FROM `xhprof_details` ";
      }
  ...
  public function getDistinct($data)
  {
	...
	$query = "SELECT DISTINCT(`{$sql['column']}`) FROM `xhprof_details`";
  ...
  */
  public function get_run($run_id, $type, &$run_desc)
  {
    ...
    $query = "SELECT * FROM `xhprof_details` WHERE `id` = '$run_id'";
    $resultSet = $this->db->query($query);
  ...
  */
  public function getRunComparativeData($url, $c_url)
  {
      ...
      $query = "SELECT count(`id`), avg(`wt`), min(`wt`), max(`wt`),  avg(`cpu`), min(`cpu`), max(`cpu`), avg(`pmu`), min(`pmu`), max(`pmu`) FROM `xhprof_details` WHERE `url` = '$url'";
      $rs = $this->db->query($query);
      ...
      //Runs same c_url
      //  count, avg/min/max for wt, cpu, pmu
      $query = "SELECT count(`id`), avg(`wt`), min(`wt`), max(`wt`),  avg(`cpu`), min(`cpu`), max(`cpu`), avg(`pmu`), min(`pmu`), max(`pmu`) FROM `xhprof_details` WHERE `c_url` = '$c_url'";
      $rs = $this->db->query($query);
      $row = $this->db->getNextAssoc($rs);
      $row['url'] = $c_url;
  ...
  protected function calculatePercentile($details)
  {
                  ...
                  $query = "SELECT `{$details['column']}` as `value` FROM `xhprof_details` WHERE `{$details['type']}` = '{$details['url']}' ORDER BY `{$details['column']}` DESC LIMIT $limit, 1";
                  $rs = $this->db->query($query);
                  $row = $this->db->getNextAssoc($rs);
                  return $row['value'];
  }
    ...
    public function save_run($xhprof_data, $type, $run_id = null, $xhprof_details = null)
    {
		...
		*/
		...
		if (!isset($GLOBALS['_xhprof']['serializer']) || strtolower($GLOBALS['_xhprof']['serializer'] == 'php')) {
		    ...
		    if (isset(xhprof['savepost']) && xhprof['savepost'])
			{
